...
// IOExpander Address
//#define 8574_ADDR 0x20

// 

// Motor control digital output pins defined as global constants (4 wheel drive with 2 Lego motors):
const int controlPin1A = 7;                  // L293D driver input 1A on pin no 2  http://www.ti.com/lit/ds/symlink/l293.pdf connected to Arduino digital output pin 2
const int controlPin2A = 8;                  // L293D driver input 2A on pin no 7 connected to Arduino digital output pin 5
...
const int controlPin3A = 13;                  // L293D driver input 3A on pin no 10 connected to Arduino digital output pin 6
const int controlPin4A = 12;                  // L293D driver input 4A on pin no 15 connected to Arduino digital output pin 8 
const int servoENablePin = 6;                // L293D ENable(3,4) input on pin no 9 connected to Arduino digital output pin 3
...
//   char* RCspeed = strtok(NULL, ",");           // next comes RCspeed which is in the Range: -100 (full speed reverse).. 0 .. 100 (full speed forward)
   int iRCsteering =0; // = atoi(RCsteering);          // convert string RCsteering into integer iRCsteering
   int iRCspeed =0 ;  // = atoi(RCspeed);                // convert string RCspeed into integer iRCspeed
/*
